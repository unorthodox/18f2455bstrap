...
void InitUSB(void) {
	...
	PORTB = 0x00;				// set bit zero to indicate Powered status
#endif
...
void ServiceUSB(void) {
	...
	if (UIRbits.UERRIF) {
		...
		BD15O.bytecount = MAX_PACKET_SIZE;
		BD15O.address = EP15_OUT_buffer;	// EP0 OUT gets a buffer
		BD15O.status = 0x88;				// set UOWN bit (USB can write)
		BD15I.address = EP15_IN_buffer;	// EP0 IN gets a buffer
		BD15I.status = 0x08;				// clear UOWN bit (MCU can write)
...
static void stdreq_get_status(void) {
	unsigned char *UEP;
	unsigned char n;
...
static void stdreq_feature(void) {
	unsigned char *UEP;
	unsigned char n;
	BUFDESC *buf_desc_ptr;
